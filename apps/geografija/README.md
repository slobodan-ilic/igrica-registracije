# Geografija Srbije — kviz

Okruzi, rivers, mountains and spas on a map of Serbia. You get a name, you click
where it is.

Part of the [kvizovi](../..) workspace: the map, scoring, gestures and design
come from `@kviz/engine`. This app supplies four topics and their datasets.

```sh
npm run dev -w @kviz/geografija
npm run build:okruzi   # and :rivers :planine :banje :relief :slike
```

## Topics

Four, picked on the menu:
...
cuts a 640px card and a 160px tile from it. Coverage here is 28/28 rivers,
24/26 spas and 23/24 mountains; the rest simply render without one.
...
Every topic here carries a Kosovo set, and one switch on the menu governs all
of them. The plate app has the same switch, over the 7 codes Serbia lists for
the province. It is **off by default** and remembered per browser. What it adds:
