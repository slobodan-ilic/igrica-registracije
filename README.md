...
**The schema comes first when a round has grown a column.** `api/rounds.js`
names every column it inserts, so a deploy that knows about `practice` while
the database does not turns every sync into a 500 — for everyone signed in, at
once. [`apps/tablice/scripts/schema.sql`](apps/tablice/scripts/schema.sql) is
the whole database and is written to be re-runnable, so applying it against the
Neon branch is the step, and it is safe to repeat:

```sh
psql "$DATABASE_URL" -f apps/tablice/scripts/schema.sql
```

Same shape as the analytics switch above: the thing the code depends on has to
exist before the code that depends on it is live.
