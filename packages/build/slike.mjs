...
import { leadImages, fileInfo, download, toCyrillic } from './wiki.mjs'
...
/**
 * Fetch a photograph for every answer in `topics`, which maps a topic id to the
 * dataset it reads. `app` is the package directory the data and images live in.
 */
export async function buildPhotos({ app, topics }) {
  const report = { used: 0, none: [], badOverride: [] }

  for (const [topic, file] of Object.entries(topics)) {
    const features = JSON.parse(
      readFileSync(resolve(app, `data/${file}.json`), 'utf8'),
    ).features
    ...
    const credits = {}
    const dir = resolve(app, 'public/img', topic)
    ...
    writeFileSync(
      resolve(app, `data/slike-${topic}.json`),
...
/** Print what happened, and fail the build on the failures that are ours. */
export function reportPhotos(report, minimum = 1) {
  ...
  // A typo in ARTICLE is the one failure that looks exactly like "this place
  // has no photograph", so it ends the build rather than hiding in the output.
  if (report.badOverride.length) {
    console.error('\nARTICLE overrides that are not real articles:')
    for (const line of report.badOverride) console.error(`    ${line}`)
    process.exit(1)
  }

  // A broken API would otherwise quietly empty every manifest.
  if (report.used < minimum) {
    console.error(`\nOnly ${report.used} photos — expected at least ${minimum}. Refusing to write that.`)
    process.exit(1)
  }
